...
def read_loan() -> tuple[float, float, float]:
    p, rate, payment = 0, 0, 0
    try:
        p = float(input('Please enter an amount to borrow: $'))
        rate = float(input('Please enter the interest rate, in percent: '))
        payment = float(input('Please enter the loan payment: $'))
        if (not p > 0) or (not rate > 0) or (not payment > 0):
            raise ValueError
    except ValueError:
        print('The balance, rate, and payment must all be positive numbers, with no dollar sign, commas, or percent sign.')
    else:
        print('Borrowing $', p, 'at', rate, "% with a payment of $", payment)
    # Loan amount, annual interest rate, number of loan periods
    return p, rate, payment

def calc_balances(p: float, rate: float, payment: float):
...
def main(args: list[str]) -> int:
    ...
    p, rate, payment = read_loan()
    if p > 0 and rate > 0 and payment > 0:
        balances: list[float] = calc_balances(p, (rate/12)/100, payment)
